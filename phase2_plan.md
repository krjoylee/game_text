...
### ⚠️ [오답 노트 / 안 좋은 케이스 기록 (Failure Cases)]
* **CASE-01 (2026-09-03): 극단적 솔리드 배경색 블록 채우기의 실패**
  * **시도**: 문자 기호의 흰색 빈틈을 없애기 위해 전면 배경색 ANSI(48;5;Nm)로 꽉 채운 2칸 도트 방식 시도.
  * **결과/피드백**: 눈·코·입의 미세 디테일이 뭉개지고, 전체가 시커멓게 칠해져 오히려 눈도 안 보이고 형태 식별이 더 어려워짐.
  * **교훈**: 8비트 게임 감성의 핵심은 '단순 무식한 덩어리 칠'이 아니라, **"선명한 외곽선 테두리 + 상하 Half-Block(▀▄)을 활용한 촘촘한 해상도 + 눈동자의 또렷한 콘트라스트"**임.
...
1. **1단계: 이미지 엔진(`AsciiArt Studio`) 혁신 (최우선 착수)**
   * `palette.py`: 16색 고전 인덱스 컬러 팔레트 정의.
   * `edge_composite.py`: 굵고 또렷한 외곽선 보존 필터 구현.
   * `crop_focus.py`: 인물 상반신 70% 집중 크롭 자동화.
2. **2단계: 흥부놀부전 1막 씬을 통한 가독성 검증**
   * 놀부의 심술궂은 눈/갓과 흥부의 서글픈 표정이 **"단 0.1초 만에 눈에 들어오는지"** 사용자 실기 검증.
3. **3단계: Rust 게임 엔진(`Divina Ludus`) 런타임 컬러화 & 1MB 방어**
   * `ScreenBuffer`의 `Cell`에 색상 인덱스(`color_idx: u8`) 추가.
   * 1MB 이하 바이너리 용량 유지 (정적 musl 빌드).
4. **4단계: 전체 팩 컬러화 및 배포 체계 갱신**
   * Windows 및 Linux/WSL 배포 런처 연동.
